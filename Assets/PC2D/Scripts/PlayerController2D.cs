...
public class PlayerController2D : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // EDIT NOTE: Changed GetButtonDown to GetButtonUp, to only do
        if (!_motor.IsFalling() && Input.GetButtonDown(PC2D.Input.JUMP))
        {
            _motor.Jump();
        ...
        {
            // Normalized movement (starts moving after a little while.
            //_motor.normalizedXMovement = Input.GetAxis(PC2D.Input.HORIZONTAL);
            // Instant movement
            _motor.normalizedXMovement = 1 * Mathf.Sign(Input.GetAxis(PC2D.Input.HORIZONTAL));
        }
        else
        {
