...
public class PlatformerMotor2D : MonoBehaviour
{
    ...
    /// <summary>
    /// Delegate to attach to when the motor makes a regular jump (called before onJump).
    /// </summary>
    public Action onNormalJump;
    ...
    /// <summary>
    /// Delegate that notifies that the motor has landed after falling (wallsliding NOT included). amountFallen can be queried for distance fallen.
    /// </summary>
    public Action onLanded;

    /// <summary>
    /// Delegate that notifies that the motor reaches the ground.
    /// </summary>
    public Action onGrounded;

    /// <summary>
    /// Delegate that notifies that the motor has begun sticking to a wall.
    /// </summary>
    public Action onWallSticking;

    /// <summary>
    /// Delegate that notifies that the motor has begun sliding down a wall.
    /// </summary>
    public Action onWallSliding;

    /// <summary>
    /// Delegate that notifies that the motor has begun sliding down a wall.
    /// </summary>
    public Action onCornerGrabbed;
    ...
    /// <summary>
    /// Set the appropriate stuff for a jump being performed, without the motor taking care of the jump mechanics.
    /// </summary>
    public void ReportCustomJumpExecuted(bool disableAirJumpsUntilLanded = false)
    {
        // Set last jump type (sets the type by your current state e.g. grounded, onwall, oncorner...
        SetLastJumpType();

        // Disable airjumps
        if(disableAirJumpsUntilLanded)
            _jumping.numAirJumps = numOfAirJumps;

        // Set state to falling, to avoid the motor handling anything about the actual "jump".
        ChangeState(MotorState.Falling);
    }
    ...
    // NOTE: CHECK_TOUCHING_TRIM was originally 0.01f. Changed to 0.014f because we experienced falling through one-way platforms a lot.
    // We have tried using both BoxCollider2D and EdgeCollider2D on our platforms.
    // It's the code in GetClosestHit concerning the one-way colliders (maybe also other colliders?), where the culprit is to be found.
    // Sometimes (think "floating-point precision"-often) it will find hits[i].collider to be overlapping our player (numOfNoDistanceHits = GetOverlappingColliders() resulted
    // in 1, while with the working platforms it resulted in 0), which makes isTouching be set to true. If the collider is overlapping/touching the player collider,
    // then the player is considered to not be hitting the collider, but instead he is considered falling through it, without ever landing on it, despite that he should have.
    // Is his velocity not being taken into account properly? Is GetOverlappingColliders() buggy?
    // Increasing CHECK_TOUCHING_TRIM makes the problem MUCH less frequent, but also results in the contact-points to be more inaccurate, so the player is positioned at a
    // different height each time his y-position is corrected to match the contact-point.

    private const float CHECK_TOUCHING_TRIM = 0.014f;
    ...
    private CollidedSurface CheckGround(float distance, bool forceDistance = false)
    {
        ...
        RaycastHit2D closestHit = GetClosestHit(_collider2D.bounds.center, Vector3.down, distance + Mathf.Epsilon);
    ...
    private void ChangeState(MotorState newState)
    {
        ...
        if (motorState == MotorState.OnGround && onGrounded != null)
            onGrounded();
    }
